...
class CroSA(nn.Module):
    def __init__(self,embed_dim, dropout_r=0.2, head=8):
        super(CroSA, self).__init__()
        self.embed_dim = embed_dim
        self.dropout_r = dropout_r
        self.head = head
        self.mhatt_cross = MHAtt(self.embed_dim, self.dropout_r, self.head)
        self.ffn = build_mlp(self.embed_dim, self.embed_dim)
    ...
    def forward(self, x, y, x_mask=None, y_mask=None):
        y = self.norm1(y + self.dropout1(self.mhatt_cross(y, y, x, y_mask)))
...
class MHSA(nn.Module):
    def __init__(self, embed_dim, dropout_r=0.2, head=8):
        ...
        self.dropout_r = dropout_r
        self.head = head
        self.mhatt = MHAtt(self.embed_dim, self.dropout_r, self.head)
        self.ffn = build_mlp(self.embed_dim, self.embed_dim)
        self.dropout1 = nn.Dropout(self.dropout_r)
        self.norm1 = nn.LayerNorm(self.embed_dim)
        self.dropout2 = nn.Dropout(self.dropout_r)
...
class MHAtt(nn.Module):
    ...
    def forward(self, v, k, q, mask=None):
        ...
        bs = q.size(0)
        v = self.linear_v(v).view(bs, -1, self.head, self.embed_dim // self.head).transpose(1, 2)
        k = self.linear_k(k).view(bs, -1, self.head, self.embed_dim // self.head).transpose(1, 2)
        q = self.linear_q(q).view(bs, -1, self.head, self.embed_dim // self.head).transpose(1, 2)
        # print('q_linear: {}'.format(q.shape))
        atted = self.att(v, k, q, mask)
        # print('atted: {}'.format(atted.shape))
        atted = atted.transpose(1, 2).contiguous().view(bs, -1, self.embed_dim)
        atted = self.linear_merge(atted)
        return atted

    def att(self, v, k, q, mask=None):
        d_k = q.shape[-1]
